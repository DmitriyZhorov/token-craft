...
class UserProfile:
    ...
    def __init__(
        self, user_email: Optional[str] = None, profile_dir: Optional[Path] = None
    ):
        """
    ...
    def _create_new_profile(self) -> Dict:
        ...
        return {
            ...
            "migration": None,
            # Preferences
            "preferences": {"theme": "space_exploration", "notifications": True},
        }
    ...
    def update_from_analysis(self, score_data: Dict, rank_data: Dict):
        ...
        self.data["scores"] = {
            "token_efficiency": score_data["breakdown"]
            .get("token_efficiency", {})
            .get("score", 0),
            "optimization_adoption": score_data["breakdown"]
            .get("optimization_adoption", {})
            .get("score", 0),
            "improvement_trend": score_data["breakdown"]
            .get("improvement_trend", {})
            .get("score", 0),
            "best_practices": score_data["breakdown"]
            .get("best_practices", {})
            .get("score", 0),
            "cache_effectiveness": score_data["breakdown"]
            .get("cache_effectiveness", {})
            .get("score", 0),
            "tool_efficiency": score_data["breakdown"]
            .get("tool_efficiency", {})
            .get("score", 0),
            "session_focus": score_data["breakdown"]
            .get("session_focus", {})
            .get("score", 0),
            "cost_efficiency": score_data["breakdown"]
            .get("cost_efficiency", {})
            .get("score", 0),
            "learning_growth": score_data["breakdown"]
            .get("learning_growth", {})
            .get("score", 0),
            "waste_awareness": score_data["breakdown"]
            .get("waste_awareness", {})
            .get("score", 0),
        }
        ...
        if "token_efficiency" in score_data.get("breakdown", {}):
            ...
            self.data["avg_tokens_per_session"] = details.get(
                "avg_tokens_per_session", 0
            )
        ...
        if self.data.get("total_messages", 0) == 0:
            ...
            self.data["total_messages"] = (
                self.data.get("total_sessions", 0) * 10
            )  # Rough estimate
    ...
    def add_achievement(self, achievement_id: str, title: str, description: str):
        ...
        # Ensure achievements is a list of dicts
        if not isinstance(self.data["achievements"], list):
            self.data["achievements"] = []

        # Clean up any non-dict entries
        self.data["achievements"] = [
            a for a in self.data["achievements"] if isinstance(a, dict)
        ]

        # Don't add duplicates
        if not any(a.get("id") == achievement_id for a in self.data["achievements"]):
    ...
    def get_achievements(self) -> list:
        """Get all achievements (cleaned up to ensure valid dicts)."""
        achievements = self.data.get("achievements", [])
        # Filter out non-dict entries for safety
        return [a for a in achievements if isinstance(a, dict)]

    def get_rank_history(self) -> list:
        """Get rank change history."""
        return [
            h for h in self.data.get("history", []) if h.get("event") == "rank_change"
        ]
